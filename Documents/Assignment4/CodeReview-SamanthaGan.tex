...
rerun and bike parking, mounting, and dismounting all ran normally. All existing tests passed.
...
isOnBike = false;
// After
isOnBike = false; // bikeParked removed; use !isOnBike instead
\end{lstlisting}

\smallskip

\textbf{Smell 2: Data Clump} \quad \textbf{Commit:} \texttt{e0d5641} \\
In \texttt{EndScreen.java}, the \texttt{DIGIT\_} and \texttt{TIME\_} constants 
were grouped into a \texttt{DigitLayout} structure so that related values 
(position, size, gap) are encapsulated together rather than stored as separate 
unrelated fields. This also reduced the amount of similar code definitions in the beginning.  The end screen was tested and displayed correctly after the change. All existing tests passed.

\begin{lstlisting}
// Before
private static final float DIGIT_X = 348f, DIGIT_Y = 460f;
private static final float DIGIT_W = 150f, DIGIT_H = 140f, DIGIT_GAP = 139f; // ... repeat for all adjustments and TIME_ 
// After
private static final DigitLayout SCORE_LAYOUT = new DigitLayout(348f, 460f, 150f, 140f, 139f);
private static final DigitLayout TIME_LAYOUT  = new DigitLayout(450f, 328f, 50f, 50f, 35f);
\end{lstlisting}

\smallskip

\textbf{Smell 3: Code Duplication} \quad \textbf{Commit:} \texttt{b39dd53} \\
...
extracted into a private helper method \texttt{digitsOf()}. Both arrays now call the shared method 
instead of repeating the same operations over and over. The end screen was tested and displayed 
correct score and time values.

\begin{lstlisting}
// Before: repeated in both scoreDigits and timeDigits
(s / 1000) % 10, (s / 100) % 10, ...
// After: shared helper
...
\textbf{Smell 4: Dead Code} \quad \textbf{Commit:} \texttt{26ee304} \\
In \texttt{House.java}, the unused import, \texttt{import com.badlogic.gdx.Gdx}, was removed. No behaviour changed 
as the import was never referenced. The project was rebuilt successfully, and all existing tests passed.
