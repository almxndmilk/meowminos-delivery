...
\begin{lstlisting}
boolean blockedX = ... if (!blockedX) position.x = newX;
boolean blockedY = ... if (!blockedY) position.y = newY;
\end{lstlisting}
...
After receiving the review, I refactored each identified code smell as follows.

\textbf{Smell 1: Unnecessary Variable} \quad \textbf{Commit:} \texttt{YOUR\_COMMIT} \\
...
In \texttt{EndScreen.java}, the repetitive digit-rendering code was reduced 
using static variables to eliminate redundancy. The end screen was tested and 
displayed scores and times correctly after the change.

\textbf{Smell 3: Code Duplication} \quad \textbf{Commit:} \texttt{7208b9f} \\
In \texttt{EndScreen.java}, the repeated digit-extraction pattern used by both 
\texttt{scoreDigits} and \texttt{timeDigits} was reduced by extracting the shared 
logic. Both now use the same pattern, eliminating duplication. The end screen 
was tested and displayed correct values.
